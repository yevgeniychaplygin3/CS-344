...
void writeFile(struct movie *list, int year, int fd)
{
    ...
    strcat(title, "\n");
    int bytesWrote = write(fd, title, strlen(title));
    printf("bytes: %d\n", bytesWrote);
}
...
int main(int argc, char *argv[])
{
    ...
    char fileName[]= "movies_sampleLong_1.csv";
    struct movie *list = parseData(fileName);
    createDir(fileName, list);
    // STOPPED HERE


    //create a file for each year from the file name in the newly created directory. set permissions to rw- r-- ---. Name the file yyyy.txt

    // write to the files the titles of the movies
